...
- `manzai`/`rakugo`等の演芸系セッションが議論調の長文化・ラウンド数無視で終わらなくなる問題を修正（`max_chars`/`max_rounds`の毎ターン再掲）
...
### セキュリティ
- マルチプレイヤー機能の認証・認可を攻撃者視点で包括監査（外部QAレビュー分含む）。認証トークン漏洩・BOLA・無認証エンドポイント公開漏れ等、発見した21件のうち20件をコード修正、残り1件（Cloudflareエッジ層のレート制限設定）はインフラ側の設定手順として文書化
...
### テスト
- マルチプレイヤー関連の新規テストを多数追加（BOLA・レート制限・退室/切断・ロビー割付け・持ち込みキャラ審査 等）、E2Eシナリオ（`frontend/e2e/`）を6本追加
