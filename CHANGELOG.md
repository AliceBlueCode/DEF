...
### 新機能
- 招待コードでのオンラインセッション参加に、TERMS.md同意を実効化する専用
  オンボーディング画面（招待コード→規約同意→キャラ選択）を新設。参加後の
  ゲスト画面はサイドバー・他タブを持たない最小シェルに変更
- TRPGモードのゲスト参加時に、持ち込みキャラクターに対応するゲームシートを
  選べるように対応
- 投票による追放（vote_expel）に弁明ラウンド・参加者ごとの個別投票権・AI
  引き継ぎ確認・WS切断時の挙動修正を追加
- セッションルールの質的な指示（対立を恐れるな等）を、履歴が伸びても薄れない
  よう毎ターン再掲する`reinforced_rules`を追加
- VOICEVOXのメモリ肥大化に対する自動再起動ウォッチドッグを追加

### バグ修正（マルチプレイ・セッション進行）
このリリースの主軸。TRPGモードのAIキーパー（ゲームマスター）まわりで、
「発火した本人のタブにしか結果が届かない」「特定条件でキーパーが永久に発火
しなくなる」系の不具合が連鎖して見つかり、都度その場しのぎで直すのではなく
根本原因（`session_turn_engine.py`の巨大な共有状態を複数箇所が独立に
重複実装していた構造）まで掘り下げて対処した:
- TRPGキーパーが、セッションを作成したタブ以外では永久に発火しなくなる
  不具合を修正
- キーパー発言・ダイス判定結果が、それを発火させた本人のタブにしか表示されず
  他の参加者タブへ伝搬しない不具合を修正（`KEEPER_NARRATED`/
  `JUDGMENT_RESOLVED`のWSブロードキャストを新設）。あわせて、キーパーの
  LLMバックエンド解決がセッションの実際の設定を見ずタブごとに変動する値へ
  フォールバックしていた不具合、複数タブがほぼ同時にキーパーを発火させた際の
  二重生成防止ロックが実際には機能していなかった不具合も修正
- 人間プレイヤーの多重送信ガードが、比較対象（`session["round"]`）の非同期
  遅延更新により間欠的に機能しなくなっていたレース条件を修正
- `session["turn"]`/`session["round"]`/`session["_round_seq"]`等の
  巻き戻り正規化ロジックが4箇所以上で独立に重複実装されていた構造を
  `_normalize_round_turn`へ集約。GMの強制スキップ・「やり直し」（retake）
  機能でラウンド完了判定が壊れる欠陥、retakeが同期関数のままイベントループ外で
  非同期タスクを起動し500エラーになる実バグもあわせて修正
- AIキャラ/キーパーのLLMバックエンド個別設定（`char_backends`）を空に戻しても
  以前の設定が消えず残り続ける不具合を修正
- バックエンド自動起動時のPIDファイルガードにTOCTOUレース・空ファイル競合・
  PID再利用誤検知があった問題を修正
- 公開ポート（Cloudflare Tunnel）経由ゲストのキャラクター表示色
  （`imageColor`）が構造的に未対応だった問題を修正

### リファクタリング
- `gm_agent.py`のシステムプロンプトを`data/session_prompts.json`へ外出しし、
  判定要否の決定ロジックをナレーション生成から分離
- `session_turn_engine.py`のturn/round状態書き込みを`_normalize_round_turn`
  へ集約する前に、現状挙動を固定するキャラクタライゼーションテストを新設
  （`tests/test_regression_turn_round_state.py`、29件）

### CI
- マルチプレイヤーe2eテストをGitHub Actionsへ統合。不安定要因（実行順序依存・
  自動進行トグルの初期状態・タイムアウト不足・`browser.close()`未保証等）を
  個別に解消

### 依存関係
- Python/npm/GitHub Actions、多数の依存を更新（詳細はコミット履歴参照）
