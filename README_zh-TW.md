...
本系統不追求 AI 模仿人類。評估標準不是「像不像人」，而是：

- 能否以該角色的身份保持一致的行為
- 能否與所處世界觀保持整合性
- 判斷是否具有一貫性

**身為 AI 不是缺點——而是角色個性的一部分。**
...
透過規則書、劇本與 GM Agent，AI 角色能夠自律地進行完整的 TRPG 場次。
骰子投擲、判定、角色卡管理皆自動化處理——你可以作為守秘人管理桌面，也可以作為玩家親自加入。

### Novel — 與角色一起書寫故事

結合地文、對話與插圖，支援小說創作。
AI 候選生成・TTS 朗讀・T2I 插圖協同運作，讓你的角色以故事主角的身分持續存在。
...
只要使用外部 API（Gemini / OpenAI / Anthropic 等），無需 GPU 即可進行文字生成與語音合成。圖像生成則需要 T2I 用的 API（OpenAI DALL-E / Civitai / Hugging Face）或本地 GPU 環境。
...
- **設計審查・安全審查：** [Grok](https://grok.com/) (xAI)
- **諮詢：** [Gemini](https://gemini.google.com/) (Google) ・ [Copilot](https://copilot.microsoft.com/) (Microsoft)
