...
| F-9 | i18n・多言語対応 | 🔶 主要UI完了 | 164キー（日英）。LLM指示文は `session_prompts.json` で外出し管理（`_sp(key, lang)` 参照）。セッション内動的メッセージ約30箇所は未対応 |
...
本ドキュメントはv2.1.1時点の状況です。最新の状況はリポジトリのIssuesおよびリリースノートを参照してください。
