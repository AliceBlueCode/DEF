...
// 取り出すためだけにブラウザ側でbase64url decodeする(jwt_regen.js/ws_auth.jsと同じ手法)。
...
;(async () => {
  ...
  try {
    ...
    assert(!!sessionId, `session_id decoded from guest token: ${sessionId}`)
    ...
    // 直後にセッションが停止する現象として発見・修正)。実CIのbackend.logでは
    // AIターン生成自体はほぼ即座に成功していたにも関わらず、ホストタブのライブ
    // メッセージ一覧(WS経由)には30秒待っても反映されないことがあった(2026-08-23、
    // CI実測——原因はWSブロードキャスト配信側の何らかのタイミング要因と推測されるが
    // 未特定)。ライブ配信のタイミングに依存せず、サーバーの永続履歴
    // (session["history"]、GET /{session_id}で再構築される、リロード時と同じ経路)に
    // 実際に新しいターンが記録されたことを直接確認する——ホストをリロードして
    // 履歴を再取得する。
    await host.waitForTimeout(3000) // AIターン生成(LLM呼び出し)の完了を待つ猶予
    await host.reload()
    await host.waitForTimeout(1500)
    const hostTextAfterReload = await host.locator('body').innerText()
    assert(hostTextAfterReload.includes('モック応答'), 'a new AI-generated turn for the handed-over character was actually persisted to session history (session did not get stuck)')
