...
이 시스템은 AI가 인간을 모방하는 것을 목표로 하지 않습니다. 평가 기준은 「인간다움」이 아니라：

- 해당 캐릭터로서 일관된 행동을 할 수 있는가
- 세계관과의 정합성이 유지되는가
- 판단에 일관성이 있는가

**AI라는 것은 결함이 아니라, 캐릭터의 개성 중 하나로 다룹니다.**
...
### TRPG — 보기만 해도 좋다. 참여해도 좋다.

룰북・시나리오・GM 에이전트에 의해 AI 캐릭터들이 자율적으로 TRPG 세션을 진행합니다.
다이스 롤・판정・캐릭터 시트 관리가 자동화되어, 키퍼로서 테이블을 관리할 수도, 플레이어로서 참가할 수도 있습니다.
...
외부 API(Gemini / OpenAI / Anthropic 등)를 이용하면 GPU 없이도 텍스트 생성과 음성 합성이 동작합니다. 이미지 생성에는 T2I용 API(OpenAI DALL-E / Civitai / Hugging Face) 또는 로컬 GPU 환경이 필요합니다.
...
- **설계 리뷰・보안 리뷰:** [Grok](https://grok.com/) (xAI)
- **상담:** [Gemini](https://gemini.google.com/) (Google) ・ [Copilot](https://copilot.microsoft.com/) (Microsoft)
