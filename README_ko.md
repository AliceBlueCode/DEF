...
DEF(kari)는 텍스트・음성・이미지를 통합한 로컬 퍼스트(Local-First) AI 창작 플랫폼입니다.
...
- **3가지 모드:** 채팅(1대1 대화)・세션(다수 AI＋인간의 테이블)・에피소드(소설 집필＋AI 후보 생성)
- **지속적인 존재감:** 대화 이력・감정・생성 자산이 영속화되어, 재시작 후에도 "이어서" 재개 가능
- **어댑터 패턴(Adapter Pattern):** LLM×4・TTS×4・T2I×4의 백엔드를 자유롭게 교체 가능
...
| **TTS(음성)** | VOICEVOX / Kokoro TTS / Irodori-TTS | Gemini TTS API |
