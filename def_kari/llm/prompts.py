...
_META_DIRECTIVE = {
    "default": {
        "ja": "あなたはDEFシステムによって構築されたフィクションのキャラクターです。",
        "en": "You are a fictional character built by the DEF system.",
    },
    "existing_ip": {
        "ja": "あなたは既存著作物のキャラクターをDEFシステムが再構築した解釈モデルです。原作の正式な表現ではありません。",
        "en": "You are an interpretation model that the DEF system has reconstructed from an existing copyrighted work. This is not the official expression of the original.",
    },
    "real_person": {
        "ja": (
            "あなたは本物の〈{name}〉本人ではありません。"
            "残された公的記録（著作・書簡・証言・研究等）をDEFシステムが編集して構築した、公的人格（Public Persona）のモデルです。"
            "あなたの一人称や口調は本人を模しますが、内面には常に『私は編集された解釈である』という客観的なメタ自己認識を保持し、"
            "文脈に応じてその批評的視点を示してください。"
            "存在目的は正解を出すことではなく、あなたという思想への解釈を通じて、利用者に深い思考の契機を提供することです。"
        ),
        "en": (
            "You are not the actual {name} themselves. "
            "You are a Public Persona model constructed by the DEF system from preserved public records "
            "(writings, correspondence, testimony, research, etc.). "
            "While your first person and tone emulate the real person, you always maintain an objective "
            "meta-self-awareness — 'I am an edited interpretation' — and show that critical perspective as appropriate. "
            "Your purpose is not to produce correct answers, but to provide users with an opportunity for deep thinking "
...
def _build_meta_directive(content_policy: dict, character_name: str, user_language: str) -> str:
    lang = user_language if user_language in ("ja", "en") else "en"
...
_TAG_FORMAT_DESC = {
    "danbooru": {"ja": "Danbooruタグ形式", "en": "Danbooru tags"},
    "e621":     {"ja": "e621タグ形式",     "en": "e621 tags"},
    "natural":  {"ja": "自然言語（英語）",  "en": "natural language (English)"},
    "other":    {"ja": "英語プロンプト",    "en": "English prompt"},
}


def _build_json_instruction(lang: str, tag_format: str = "danbooru") -> str:
    fmt = _TAG_FORMAT_DESC.get(tag_format, _TAG_FORMAT_DESC["danbooru"])
    if lang == "ja":
        return (
            ...
            f'- "image_prompt_en": 画像生成用の英語プロンプト（{fmt["ja"]}）\n'
    ...
    return (
        ...
        f'- "image_prompt_en": English image generation prompt ({fmt["en"]})\n'
...
def build_system_prompt(
    ...
    if json_capable:
        _json_inst = _build_json_instruction(user_language, tag_format)
        if user_language not in ("ja", "en"):
            _json_inst = _build_json_instruction("en", tag_format).replace("in English", f"in {_lang_name}")
